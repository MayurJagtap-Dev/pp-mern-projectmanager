import { useState } from "react";

export default function Header() {
  const [toggleKey, setToggleKey] = useState(false);
  return (
    <nav className="w-full border border-gray-200 bg-gray-300">
      <div className="justify-between px-4 mx-auto lg:max-w-7xl md:items-center md:flex md:px-8">
        <div>
          <div className="flex items-center justify-between py-3 md:py-5 md:block">
            <div className="flex items-center font-medium text-gray-900 title-font md:mb-0">
              {/* <img
                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRa3bOnPztI-R93_jp9fqv_J_r_bCQI_mBzAUtKEARelxsbPlyj2l3RBjZVHmpzmxBRZp0&usqp=CAU"
                className="h-8 sm:h-20 rounded-full"
                alt="Logo"
              /> */}
              <span className="self-center text-xl font-semibold whitespace-nowrap dark:text-white">
                ProjectManager
              </span>
            </div>
            <div className="md:hidden">
              <button
                className="p-2 text-gray-700 rounded-md outline-none focus:border-gray-400 focus:border"
                onClick={() => setToggleKey(!toggleKey)}
              >
                {toggleKey ? (
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="w-6 h-6"
                    viewBox="0 0 20 20"
                    fill="currentColor"
                  >
                    <path
                      fillRule="evenodd"
                      d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                      clipRule="evenodd"
                    />
                  </svg>
                ) : (
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="w-6 h-6"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={2}
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M4 6h16M4 12h16M4 18h16"
                    />
                  </svg>
                )}
              </button>
            </div>
          </div>
        </div>
        <div>
          <div
            className={`flex-1 justify-self-center pb-2 md:block md:pb-0 md:mt-0 ${
              toggleKey ? "block" : "hidden"
            }`}
          >
            <ul className="items-center justify-center space-y-2 md:flex md:space-x-6 md:space-y-0">
              <li className="font-bold hover:text-blue-600">Client</li>
              <li className="font-bold hover:text-blue-600">Project</li>
            </ul>
          </div>
        </div>
      </div>
    </nav>
  );
}
